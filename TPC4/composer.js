exports.compositoresListPage = function(slist, d, gap){
        ...
        pagHTML += `
                <tr>
                    ...
                    for (let j = 0; j < gap.length; j++){
                        if (slist[i].periodo == gap[j].nome){
                            pagHTML += `
                                <a href="/periodos/${gap[j].id}">
                                ${slist[i].periodo}
                                </a>`
                        }
                    }
...
exports.compositorFormPage = function(d,gap){
    let pagHTML = `
    <!DOCTYPE html>
    <html>
        <head>
                ...
                <form class="w3-container" method="POST">
                    <fieldset>
                        ...
                        for (let i=0; i < gap.length; i++){
                            pagHTML += `<input type="radio" id="${gap[i].id}" name="periodo" value="${gap[i].nome}">
                            <label for="html">${gap[i].nome}</label><br>`
                        }
...
exports.compositorFormEditPage = function(b, d, gap){
    let a = b[0]
    var pagHTML = `
    <!DOCTYPE html>
    <html>
            ...
            <div class="w3-card-4">
                ...
                <form class="w3-container" method="POST">
                    <fieldset>
                        ...
                        console.log(gap)
                        for (let i=0; i < gap.length; i++){
                            if (a.periodo == gap[i].nome){
                                pagHTML += `<input type="radio" id="${gap[i].id}" name="periodo" value="${gap[i].nome}" checked>
                                <label for="html">${gap[i].nome}</label><br>`
                            } else {
                                pagHTML += `<input type="radio" id="${gap[i].id}" name="periodo" value="${gap[i].nome}">
                                <label for="html">${gap[i].nome}</label><br>`
...
exports.compositorPage = function(comp, d, gap){
    compositor = comp[0]
    var pagHTML = `
    <html>
    <head>
        ...
        <div class="w3-card-4">
            ...
            <div class="w3-container">
                <ul class="w3-ul w3-card-4" style="width:50%">
                    ...
                    `
                    for (let j = 0; j < gap.length; j++){
                        if (compositor.periodo == gap[j].nome){
                            pagHTML += `
                                <li><b>Period: </b> <a href="/periodos/${gap[j].id}">
                                ${compositor.periodo}
                                </a>`
                        }
                    }
